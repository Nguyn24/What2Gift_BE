...
{
    public async Task<Result<Page<GetProductResponse>>> Handle(GetProductByOccasionQuery request, CancellationToken cancellationToken)
    {
        var query = context.Products
            .Include(s=>s.ProductSources)
            .Where(p => p.OccasionId == request.OccasionId);
